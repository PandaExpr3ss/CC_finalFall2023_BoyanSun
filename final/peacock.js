let peamovex = 0; //need to change
let peamovey = 0; //need to change
...
class peacock{
    ...
    constructor(x, y, img, flip){
        ...
        this.img = img;
        this.flip = flip;
        this.peamovex = peamovex; //need to change
        this.peamovey = peamovey; //need to change
        this.d = d;
        this.v = v;
    ...
    move(){
        if(!this.strikemode){ //moves need to change
            if(frameCount % 200 == 0){
                this.peamovex = random(-2, 2);
                if(this.y <= cusMap(700, 'y')){
                    this.peamovey = random(1, 3);
                    this.movev = createVector(this.peamovex, this.peamovey);
                    this.angle = findHeadingAngle(this.movev);
                }
                else if(this.y >= cusMap(1100, 'y')){
                    this.peamovey = random(-3, -1);
                    this.movev = createVector(this.peamovex, this.peamovey);
                    this.angle = findHeadingAngle(this.movev);
                }
                else{
                    this.peamovey = random(-0.6, 0.6);
                    this.movev = createVector(this.peamovex, this.peamovey);
                    this.angle = findHeadingAngle(this.movev);
            ...
            this.x += this.peamovex;
            this.y += this.peamovey;
    ...
    strike(){
        if(!hooked){
            ...
            if(this.d <= 200 && this.d >= 20){ //striking
                ...
                this.angle = findHeadingAngle(this.movev);
                if(this.peamovex < 0){ //need to change
                    this.flip = 1;
                }
                else{
                    this.flip = -1;
            ...
            //print(this.d);
            this.angle = PI/2;
            fishtype = 'peacock'; //need to change
            if(liney <= 100){
                scrn = 'hooked'
            }
        }
