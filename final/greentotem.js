let greenmovex = 0; //need to change
let greenmovey = 0; //need to change
...
class greentotem{
    ...
    constructor(x, y, img, flip){
        ...
        this.img = img;
        this.flip = flip;
        this.greenmovex = greenmovex; //need to change
        this.greenmovey = greenmovey; //need to change
        this.d = d;
        this.v = v;
    ...
    move(){
        if(!this.strikemode){ //moves need to change
            if(frameCount % 200 == 0){
                this.greenmovex = random(-2, 2);
                if(this.y <= cusMap(700, 'y')){
                    this.greenmovey = random(1, 3);
                    this.movev = createVector(this.greenmovex, this.greenmovey);
                    this.angle = findHeadingAngle(this.movev);
                }
                else if(this.y >= cusMap(1100, 'y')){
                    this.greenmovey = random(-3, -1);
                    this.movev = createVector(this.greenmovex, this.greenmovey);
                    this.angle = findHeadingAngle(this.movev);
                }
                else{
                    this.greenmovey = random(-0.6, 0.6);
                    this.movev = createVector(this.greenmovex, this.greenmovey);
                    this.angle = findHeadingAngle(this.movev);
            ...
            this.x += this.greenmovex;
            this.y += this.greenmovey;
    ...
    strike(){
        if(!hooked){
            ...
            if(this.d <= 200 && this.d >= 20){ //striking
                ...
                this.angle = findHeadingAngle(this.movev);
                if(this.greenmovex < 0){ //need to change
                    this.flip = 1;
                }
                else{
                    this.flip = -1;
            ...
            //print(this.d);
            this.angle = PI/2;
            fishtype = 'greentotem'; //need to change
            if(liney <= 100){
                scrn = 'hooked'
            }
        }
